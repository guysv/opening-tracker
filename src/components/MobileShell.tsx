...
function tabFromSearch(search: string): MobileTabId {
  try {
...
function syncTabQueryParam(tab: MobileTabId) {
  try {
    const u = new URL(window.location.href);
    if (tab === "explorer") u.searchParams.delete(MOBILE_TAB_QUERY_KEY);
 ...
 */
export function MobileShell({ sideView, explorer, bookmarks }: MobileShellProps) {
  const [tab, setTab] = useState<MobileTabId>(() =>
    typeof window === "undefined" ? "explorer" : tabFromSearch(window.location.search),
  );
  ...
  useEffect(() => {
    try {
      ...
      syncTabQueryParam(tabFromSearch(window.location.search));
    } catch {
      /* ignore */
    }
  }, []);
